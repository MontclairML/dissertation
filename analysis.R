...
numeric_columns$DLEAVING<-as.factor(as.character(numeric_columns$DLEAVING))
...
set.seed(7)
model.glmnet<-train(DLEAVING~.,data=train_data,tuneGrid=expand.grid(alpha=0:1,lambda=seq(0.0001,1,length=20)),
                    method="glmnet", metric=metric, trControl=train.control)

set.seed(6)
model.rf<-train(DLEAVE ~., tuneLength=10, data = train_data, method = "ranger", metric=metric,
                trControl = train.control)
summary(model.rf)
set.seed(4)
